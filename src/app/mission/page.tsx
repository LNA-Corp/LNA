"use client";

import PageBrandHeader from "@/components/PageBrandHeader";
import PageShell from "@/components/PageShell";
import SubpageHero from "@/components/SubpageHero";

const PRINCIPLES = [
  "Fact is not interpretation",
  "Interpretation is not intention",
  "Understanding is not justification",
  "Understanding is not permission",
  "Explanation is not decision",
  "Prediction is not destiny",
  "AI is not agency",
] as const;

export default function MissionPage() {
  return (
    <PageShell>
      {(S) => (
        <>
          <SubpageHero
            style={S.subpageIntroWithTail}
            scrollTargetId="philosophy"
            hintMarginTop={S.scrollHintMargin}
          >
            <PageBrandHeader title="Mission" titleStyle={S.pageTitle} />
            <div style={S.quote}>
              {
                "\u201CTo help people see their own mind clearly enough to choose\u201D"
              }
            </div>
          </SubpageHero>

          <section id="philosophy" style={S.contentAfterIntro}>
            <div style={S.contentRule} aria-hidden="true" />
            <div style={S.sectionLabel}>Philosophy</div>
            <div style={{ ...S.quoteKo, marginTop: S.paragraphGapTight }}>
              {
                "\u201C\uC228\uAE30\uC9C0 \uC54A\uB294 \uAC10\uC815.\n\uAFB8\uBBF8\uC9C0 \uC54A\uC740 \uC778\uAC04.\n\uC774\uD574\uAC00 \uB9CC\uB4DC\uB294 \uD310\uB2E8\uC758 \uD574\uC0C1\uB3C4.\u201D"
              }
            </div>

            <p style={{ ...S.body, marginTop: S.sectionGap }}>
              {"\uC6B0\uB9AC\uB294 \uAC10\uC815\uC744 \uC228\uAE30\uC9C0 \uC54A\uB294\uB2E4."}
            </p>
            <p style={{ ...S.body, marginTop: S.paragraphGapTight }}>
              {
                "\uB9D0 \uC18D\uC5D0 \uC2A4\uBA70 \uC788\uB294 \uC791\uC740 \uD754\uB4EC\uB9BC\uAE4C\uC9C0\n\uADF8\uB300\uB85C \uBC1B\uC544\uB4E4\uC778\uB2E4."
              }
            </p>
            <p style={{ ...S.body, marginTop: S.paragraphGapTight }}>
              {
                "\uC2A4\uC2A4\uB85C\uB3C4 \uC54C\uC9C0 \uBABB\uD588\uB358\n\uB9C8\uC74C\uC758 \uBCC0\uD654\uAE4C\uC9C0 \uD558\uB098\uC758 \uC2E0\uD638\uB85C\n\uC77D\uB294\uB2E4."
              }
            </p>

            <p style={{ ...S.body, marginTop: S.paragraphGap }}>
              {
                "LNA Corp\uC758 \uBAA8\uB4E0 \uC2DC\uC2A4\uD15C\uC740 \uAC10\uC815\uACFC \uC0DD\uAC01\uC774 \uB4DC\uB7EC\uB098\uB294 \uBC29\uC2DD\uC744 \uAD6C\uC870\uD654\uD558\uACE0,\n\uADF8 \uC774\uD574\uAC00 \uC2A4\uC2A4\uB85C \uD310\uB2E8\uD560 \uC218 \uC788\uB294 \uC0C1\uD0DC\uB85C \uC774\uC5B4\uC9C0\uB294 \uACFC\uC815\uC744 \uC124\uACC4\uD558\uB294 \uAE30\uC220\uC785\uB2C8\uB2E4."
              }
            </p>
            <p style={{ ...S.body, marginTop: S.paragraphGapTight }}>
              {"\uC778\uAC04\uC744 \uAFB8\uBBF8\uC9C0 \uC54A\uB294\uB2E4."}
            </p>
            <p style={{ ...S.body, marginTop: S.paragraphGapTight }}>
              {"\uC778\uAC04\uC744 \uB2E8\uC21C\uD654\uD558\uC9C0 \uC54A\uB294\uB2E4."}
            </p>
            <p style={{ ...S.body, marginTop: S.paragraphGapTight }}>
              {
                "\uC778\uAC04\uC744 \uC774\uD574\uD558\uACE0, \uADF8\uB9AC\uACE0 \uC2A4\uC2A4\uB85C \uD310\uB2E8\uD558\uAC8C \uD55C\uB2E4."
              }
            </p>
          </section>

          <section style={{ ...S.block, paddingTop: S.sectionGap }}>
            <div style={S.sectionDivider}>
              <div style={S.sectionLabel}>Principles</div>
            </div>
            {PRINCIPLES.map((principle, idx) => (
              <p
                key={principle}
                style={{
                  ...S.body,
                  marginTop: idx === 0 ? S.paragraphGap : S.paragraphGapTight,
                  color: "rgba(0,0,0,0.52)",
                }}
              >
                {principle}
              </p>
            ))}
            <p
              style={{
                ...S.quoteKo,
                marginTop: S.paragraphGap,
                fontSize: 15,
                color: "rgba(0,0,0,0.58)",
              }}
            >
              {
                "The final decision belongs to you\n\u2014 \uACB0\uC815\uC740 \uB2F9\uC2E0\uC758 \uBABF\uC785\uB2C8\uB2E4."
              }
            </p>
          </section>
        </>
      )}
    </PageShell>
  );
}
